function [Xnew, Unew,Jnew] = forward(X, U, k, K,local_plan,obs_traj,arg)
    ...
    JoldForward = getTotalCost(X, U,local_plan,obs_traj,arg);
    ...
    for j = 1:max_iterations
        ...
        for i = 1:arg.N
            ...
            Xnew(i+1, :) = dynamics(Xnew(i, :), Unew(i, :),arg);
        end
        
        JnewForward = getTotalCost(Xnew, Unew,local_plan,obs_traj,arg);
...
%         error('Line search failed: No suitable alpha found after %d iterations.', max_iterations);
    ...
    Jnew = getTotalCost(Xnew,Unew,local_plan,obs_traj,arg);
